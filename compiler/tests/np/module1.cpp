...
cfg['include_dirs'] = ['/home/tong/DDARING/src/phase1-toolchain/compiler/include']
cfg['dependencies'] = ['/home/tong/DDARING/src/phase1-toolchain/compiler/include/NpArray.hpp']
setup_pybind11(cfg)
%>
...
// double sum1_3d(py::array_t<double> r) {
//   double sum = 0;
  
//   for (ssize_t i = 0; i < r.shape()[0]; i++)
//     for (ssize_t j = 0; j < r.shape(1); j++)
//       for (ssize_t k = 0; k < r.shape(2); k++)
//         sum += r.mutable_at(i, j, k);
...
PYBIND11_PLUGIN(module1) {
  pybind11::module m("module1", "auto-compiled c++ extension");
  m.def("sum", &sum);
